...
export async function getPostById(context: Context) {
    try {
        ...
        const [rows] = await pool.query<Post[]>('SELECT * FROM posts WHERE post_id = ?',[postId]);
        const data = rows[0];
...
export async function createPost(context: Context) {
    try {
        ...
        for (const post of posts) {
            const [result] = await pool.query<ResultSetHeader>('INSERT INTO posts (title, description, status) VALUES (?, ?, ?)',[post.title, post.description, post.status]);
            const postId = result.insertId;
            const [data] = await pool.query<Post[]>(
...
export async function deletePostById(context: Context) {
    try {
        ...
        await pool.query<ResultSetHeader>('DELETE FROM posts WHERE post_id = ?',[postId]);
...
export async function updatePostById(context: Context) {
    try {
        ...
        const [existingRows] = await pool.query<Post[]>('SELECT * FROM posts WHERE post_id = ?',[postId]);
        ...
        const updatedPost: UpdatePost = {title: body.title ?? existing.title, description: body.description ?? existing.description, status: body.status ?? existing.status};
        ...
        const [data] = await pool.query<Post[]>('SELECT * FROM posts WHERE post_id = ?',[postId]);
